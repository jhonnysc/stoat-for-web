# voice-channel-discord-navigation

## Purpose

Define how voice channels are presented and navigated in the client, so that voice-channel entries behave like Discord-style channel entries: activating a voice channel joins it automatically and exposes a separate accessible `Open Chat` action, while the voice-call preview banner is removed and existing voice controls are preserved.
